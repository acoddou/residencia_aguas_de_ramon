# ==========================================================================
# Script: 05_extraccion_series_landsat.R
# Descripción: Convierte el CSV regularizado de Landsat en un archivo .txt 
...
# 1. Configuración ---------------------------------------------------------
input_csv_path <- "01_data/raw/landsat_ndvi_test.csv"
output_txt_timesat_path <- "01_data/landsat_ndvi_test.txt" 
nyear <- 9
nptperyear <- 24
...
datos_csv_landsat <- readr::read_csv(input_csv_path, show_col_types = FALSE)
nts <- nrow(datos_csv_landsat)
...
if (ncol(datos_csv_landsat) != expected_total_cols_with_id) {
  warning(paste("El CSV no tiene el número de columnas esperado. Tiene:", ncol(datos_csv_landsat), 
                "Esperado:", expected_total_cols_with_id))
}

# 3. Limpieza de columnas (ID y Coordenadas) -------------------------------
# Se asume orden: id_row, x, y, NDVI_...
landsat_ndvi_values_only <- datos_csv_landsat %>%
  select(-(1:3))
...
# Escribir valores con formato de 6 decimales y un solo espacio
apply(landsat_ndvi_values_only, 1, function(fila_de_valores) {
